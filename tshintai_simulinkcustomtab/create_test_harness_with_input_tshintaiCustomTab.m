...
for i = 1:size(set_line_info, 1)
    ...
    %%
    if (evalin(harness_workspace, ...
            ['isstruct(', set_line_info{i, 3}, '.Values)']))
        evalin(harness_workspace, ...
            ['bus_obj = Simulink.Bus.createObject(', ...
            set_line_info{i, 3}, '.Values);']);

        bus_obj_name = [set_line_info{i, 3}, ...
            added_name, '_bus'];
        auto_gen_bus_name = evalin(harness_workspace, ...
            'bus_obj.busName');

        assignin('base', bus_obj_name, ...
            evalin('base', auto_gen_bus_name));

        evalin('base', ...
            ['clear(', COMMA, auto_gen_bus_name, COMMA, ');']);

        eval(['set_param(', COMMA, harness_name, '/', ...
            set_line_info{i, 3}, COMMA, ',', COMMA, ...
            'OutDataTypeStr', COMMA, ',', COMMA, ...
            'Bus: ', bus_obj_name, COMMA, ');']);
